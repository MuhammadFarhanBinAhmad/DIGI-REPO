...
With associative arrays, counting the number of times that each distinct
word occurs in a file is not difficult.
// compile like this: g++ -std=c++17 -pedantic -Wall -Wextra -Werror word-counter.cpp
// use like this: ./a.out text-file-with-words
*/
#include <iostream>
#include <fstream>
#include <map>
...
int main(int argc, char *argv[]) {
  ...
  while (ifs >> s) {
    // This is the interesting part. What happens here is that we look in
  ...
  // position within the mpa. Accordingly, the key is always const, so that if we
  ...
  // flip key/value pair
  for(std::pair<std::string, int> const& p : histogram) {
  ...
  rowctr = 1;
  for (std::pair<int, std::string> const& p : rev_histogram) {
