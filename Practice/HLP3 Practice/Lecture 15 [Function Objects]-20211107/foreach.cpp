// CSD2125 11/02
// std::for_each is the only algorithm that returns the
// (updated) function object passed to the algorithm.
// This means that we can use std::for_each algorithm to
// compute things such as averages and other types of
// summary statistics.
...
int main() {
  ...
  // find mean value
  MeanValue m = std::for_each(std::begin(vi), std::end(vi), MeanValue{});
  std::cout << "Average value: " << m.value() << "\n";
}