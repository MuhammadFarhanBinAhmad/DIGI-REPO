...
void BList<T, Size>::insert(const T &value)
{
  ...
  if (head_ == tail_ && BListStats_.ArraySize != 1)
  {
    ...
    else if (head_->count < BListStats_.ArraySize)
    {
      ...
      {
        for (size_t i = head_->count; i > 0; i--)
        {
          head_->values[i] = head_->values[i - 1];
        }
        head_->values[0] = value;
      ...
      if (value < NodePtr->values[count - 1] || NodePtr->next == NULL)
      {
        if (NodePtr->next == NULL)
        {
          ...
          if (NodePtr->prev != NULL &&
              ...
              value < NodePtr->values[0])
          {
            NodePtr->prev->values[NodePtr->prev->count] = value;
            NodePtr->prev->count++;
            BListStats_.ItemCount++;
          ...
          else if (count < BListStats_.ArraySize)
          {
            ...
            {
              for (size_t i = NodePtr->count; i > 0; i--)
              {
            ...
            {
              NodePtr->values[NodePtr->count] = value;
            }
            else
            {
        ...
        //
                 ...
                 NodePtr->count < BListStats_.ArraySize &&
                 NodePtr->prev->count < BListStats_.ArraySize &&
                 NodePtr->prev->values[NodePtr->prev->count - 1] < value && value < NodePtr->values[0])
        {
          NodePtr->prev->values[NodePtr->prev->count] = value;
          NodePtr->prev->count++;
          BListStats_.ItemCount++;
                                                          ...
                                                          const T value)
{
  ...
  if (value < SecondNode->values[0])
  {
    ...
    {
      for (size_t i = FirstNode->count; i > 0; i--)
      {
        FirstNode->values[i] = FirstNode->values[i - 1];
      }
    ...
    {
      FirstNode->values[FirstNode->count] = value;
    }
    else
    {
  ...
  else
  {
    if (value < SecondNode->values[0])
    {
      for (size_t i = SecondNode->count; i > 0; i--)
      {
        SecondNode->values[i] = SecondNode->values[i - 1];
      }
      SecondNode->values[0] = value;
    }
    else if (SecondNode->values[SecondNode->count - 1] < value)
    {
      SecondNode->values[SecondNode->count] = value;
    }
    else
    {
  ...
  SplitCheck(node, FirstNode, SecondNode);
...
/*!
\fn     template <typename T, unsigned Size>
        BList<T, Size>::SplitCheck(BNode *node, BNode *FirstNode, BNode *SecondNode)

\brief  Templated Function to check prev and next node for splitted node

\param  node - Head of the node to be inserted
\param  FirstNode - First half of splitted new node
\param  SecondNode - Second half of splitted new node

*/
/******************************************************************************/
template <typename T, unsigned Size>
void BList<T, Size>::SplitCheck(BNode *node, BNode *FirstNode, BNode *SecondNode)
{
...
/*!
\fn     template <typename T, unsigned Size>
        bool BList<T, Size>::HeadIsEmpty()

\brief  Templated Function to check if head is empty
...
/*!
\fn     template <typename T, unsigned Size>
        bool BList<T, Size>::TailIsEmpty()

\brief  Templated Function to check if tail is empty
