    ...
    */
    ...
    for (int r = 0; r < terrain->get_map_height(); r++)
    {
        for (int c = 0; c < terrain->get_map_width(); c++)
        {
            ...
            if (Vec2::Distance(Vec2(static_cast<float>(r),static_cast<float>(c)), Vec2(static_cast<float>(enemyGridPos.row),static_cast<float>(enemyGridPos.col))) < closeDistance)
            {
                GridPos enemyGridPos = terrain->get_grid_position(enemy->get_position());
            ...
            else
            {
                ...
                if (dotProd > cos((fovAngle * PI / 180.f)))
                {
    ...
    */
    ...
    for (int r = 0; r < terrain->get_map_height(); r++)
    {
        for (int c = 0; c < terrain->get_map_width(); c++)
        {
            ...
            if (std::abs(value - maximumValue) < FLT_EPSILON) // Check if the values are close enough
            {
                GridPos temp;
                temp.row = r;
                temp.col = c;
