...
auto test4() -> void {
	...
	csd2125::allocator<csd2125::vertex, short> allocator; // short is 16 bits
	using pointer = decltype(allocator)::pointer;
	pointer pv1 = allocator.allocate(10); //specify bits in short
	pointer pv2 = allocator.allocate(4); //asking for 4 more elements
	allocator.deallocate(pv1, 10); //freeing the first 10 elements
	pointer pv3 = allocator.allocate(16); //create another memory pool
	pointer pv4 = allocator.allocate(8); //check for space
