...
\brief		My attempt to create my own function which handles the rotation and movement
			...
			If you could tell me which the direction in which i could be wrong(Lack of understanding,wrong structure,overengineering(A TA told me that i was overengineering this)) that would be helpful
...
f32 GetDistance(AEVec2* Origin,AEVec2* Direction)
{
	...
	return (f32)(sqrt(pow(distance_x, 2) + pow(distance_y, 2)));
}
...
//Set and get directional normal of object
...
//Move object foward depending on which direction its facing
